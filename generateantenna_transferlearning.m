function generateantenna_transferlearning(x_dis, y_dis, samples_to_generate)

    parpool('local',8)
    iter = samples_to_generate;
    load('antenna_dataset.mat')
    tic
    ant_des = XTrain1;
    e_eff = 4.2516;
...
fmin = 1e9; fmax = 5e9; N = 81; freq = linspace(fmin,fmax,N);   
    ...
    output_file_name = ['antenna_dataset_fr4_', num2str(num_pixel), '_p_hf_',...
    ...
    parfor ii=1:samples_to_generate
         ...
         Exc1 = reshape(Exc,1,[]);
         tmp(ii,:) = Exc1; 
        p = generateantenna_scaled(nx, ny, Exc);
        figure
        try
