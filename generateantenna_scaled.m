function p = generateantenna_scaled(x_dis, y_dis, ant_des)
            tic
            e_eff = 4.2516; %For FR4; CHANGE AS PER ROGERS
            a = 4; %scaling factor
                % a = 1.94;

            L = a*((2*3.75e-3)/sqrt(e_eff)) % x axis //Conventional size for resonating at %5 GHz
            W = a*((2*3.75e-3)/sqrt(e_eff)) % y axis
    ...
    % the feedsection
      cfed = cboard+feed;
      thick = 3.2e-3; %thickness
        p = pcbStack;
        d = dielectric('Name','FR4','EpsilonR',4.8,...
            'LossTangent',0.0260,'Thickness',thick);
